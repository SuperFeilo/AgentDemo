"""ANATOMY COMPONENT: LOOP (agent #3a — Submissions Quality)

Generator pattern identical to cost_agent/loop.py: THINK -> request
tool call -> OBSERVE, harness injects tool results via .send(). Every
finding is journaled to the blackboard with its data origin.
"""
...
def submissions_loop(subject: dict, plan, brain):
    """Evaluate one submission/segment (dict with submission_id or
    broker/class). Yields events; receives tool results via .send()."""
    bb = CaseBlackboard()
    ctx = {"subject": subject}
    ...
    bb.write("case", "subject", subject,
             f"submission {subject.get('submission_id', subject.get('broker', '?'))}",
